...
test "multi-range request is rejected with 416 and proper Content-Range" {
    ...
    try std.fs.cwd().writeFile(.{ .sub_path = "tmp_range.txt", .data = "abcdefghij" });

    var resp = try tnet.httpGet(A, port, "GET", "/tmp_range.txt", &.{
        "Range: bytes=0-0,2-5\r\n",
    });
    defer A.free(resp.raw);

    try testing.expectEqual(@as(u16, 416), resp.status);
    const cr = tnet.headerValue(resp.headers, "Content-Range") orelse return error.MissingContentRange;
    try testing.expect(std.mem.startsWith(u8, cr, "bytes */10"));

    tnet.stopServer(port);
}

test "single-range 206 still ok and header casing is stable" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer std.debug.assert(gpa.deinit() == .ok);
    const A = gpa.allocator();
    ...
    try std.fs.cwd().writeFile(.{ .sub_path = "tmp_range_ok.txt", .data = "klmnopqrst" });

    var resp = try tnet.httpGet(A, port, "GET", "/tmp_range_ok.txt", &.{
        "Range: bytes=3-6\r\n",
    });
    defer A.free(resp.raw);

    try testing.expectEqual(@as(u16, 206), resp.status);
    try testing.expectEqualStrings("mnop", resp.body);

    const hdrs = resp.headers;
    try testing.expect(std.mem.indexOf(u8, hdrs, "Content-Type: ") != null);
    try testing.expect(std.mem.indexOf(u8, hdrs, "Cache-Control: ") != null);
    try testing.expect(std.mem.indexOf(u8, hdrs, "ETag: ") != null);
    try testing.expect(std.mem.indexOf(u8, hdrs, "Accept-Ranges: ") != null);
